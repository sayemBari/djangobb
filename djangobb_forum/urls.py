...
from djangobb_forum.forms import PersonalProfileForm, MessagingProfileForm, PersonalityProfileForm, \
...
urlpatterns = [
    ...
    url('^topic/(?P<topic_id>\d+)/stick_unstick/(?P<action>[s|u])/$', forum_views.stick_unstick_topic,
        name='stick_unstick_topic'),
    url('^topic/(?P<topic_id>\d+)/open_close/(?P<action>[c|o])/$', forum_views.open_close_topic,
